...
def insert_city(n):
  ...
  values = []
  for _ in range(n):
    values.append((fake.unique.city(), 
...
def insert_address(n, max1):
  values = []
  for _ in range(n):
    values.append((fake.building_number(), 
                   fake.street_name(),
                   fake.street_prefix(),
                   random.randint(1, max1),
                  ))
  insert_many("address", "street_number, street, address_complement, id_city", values, "%s, %s, %s, %s")
...
def insert_advisor(n, max1):
  ...
  insert_many("advisor", "first_name, last_name, status, id_address", values, "%s, %s, %s, %s")

# id_agency, id_advisor
def insert_agency_advisor(n, max1, max2):
  values = []
  for _ in range(n):
    values.append((random.randint(1, max1), 
...
def insert_client(n, max1, max2):
  ...
  insert_many("client", "first_name, last_name, id_address, id_agency", values, "%s, %s, %s, %s")

# id, balance, iban, swift, id_client
def insert_account(n, max1):
  values = []
  for _ in range(n):
    values.append((round(random.uniform(100.00, 100_000.00), 2),
                   fake.unique.iban(),
                   fake.swift8(),
                   random.randint(1, max1),
                  ))
  insert_many("account", "balance, iban, swift, id_client", values, "%s, %s, %s, %s")
...
def insert_loan(n, max1):
  ...
  for _ in range(n):
    ...
    values.append((round(random.uniform(100.00, 10_000.00), 2),
  ...
  insert_many("loan", "balance, interest_rate, opening, ending, risk, id_account", values, "%s, %s, %s, %s, %s, %s")
...
def insert_saving_account(n, max1, max2):
  ...
  insert_many("saving_account", "balance, interest_rate, id_account, id_product", values, "%s, %s, %s, %s")
...
def insert_transaction(n, max1, max2):
  ...
  insert_many("transaction", "amount, label, id_emitter, id_receiver", values, "%s, %s, %s, %s")
